...
REGISTER_OP("BitwidthReshapePos")
    .Attr("fixed_size: int")
    .Attr("fixed_prec: int")
    .Attr("T: {float, double}")
    .Input("to_reshape: T")
...
template<typename T>
class BWReshapeOp : public OpKernel {
 public:
  explicit BWReshapeOp(OpKernelConstruction* context) : OpKernel(context) {
    // Get Attributes
  ...
  void Compute(OpKernelContext* context) override {
    ...
    // rounds towards zero (e.g. -0.001 -> 0 )
	    T fixed_number = floor(std::abs(input(i))*(1UL<<fixed_prec)) / (1UL<<fixed_prec) *(input(i)<0?(-1):1);
#else
