...
@slim.add_arg_scope
def bottleneck(inputs, depth, depth_bottleneck, stride, conv2d, max_pool2d, rate=1,
  ...
  with tf.variable_scope(scope, 'bottleneck_v1', [inputs]) as sc:
    ...
    if depth == depth_in:
      shortcut = resnet_utils.subsample(inputs, stride, max_pool2d, scope='shortcut')
    else:
      shortcut = conv2d(inputs, depth, [1, 1], stride=stride,
                             activation_fn=None, scope='shortcut')

    residual = conv2d(inputs, depth_bottleneck, [1, 1], stride=1,
                           scope='conv1')
    residual = resnet_utils.conv2d_same(residual, depth_bottleneck, 3, stride, conv2d,
...
def resnet_v1(inputs,
  ...
  with tf.variable_scope(scope, 'resnet_v1', [inputs], reuse=reuse) as sc:
    ...
    with slim.arg_scope([slim.conv2d, QConv.conv2d, bottleneck,
      ...
      with slim.arg_scope([slim.batch_norm, QBatchNorm.batch_norm], is_training=is_training):
        net = inputs
