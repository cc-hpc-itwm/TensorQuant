...
class NM02(optimizer.Optimizer):
  ...
  def __init__(self,
    ...
    """
    super(NM02, self).__init__(use_locking, name)
  ...
  def _apply_rmsprop(self,var,
        ...
        grad_counter_new = tf.minimum(tf.maximum(grad_counter+tf.sign(grad),-8),8)
        #skip 0
        grad_counter_new += tf.sign(grad)*tf.cast(grad_counter_new == 0,tf.float32)
        
        rms = rms.assign( (tf.square(grad)-rms) * (1 - decay_tensor) + rms )
        mom = mom.assign( (mom * momentum_tensor + (grad * learning_rate_tensor)) / 
                    (tf.sqrt(rms + epsilon_tensor)) )
        delta = tf.abs(grad_counter_new) * mom
        var_new = quantizer.quantize(var-delta)
        #var_new = var-delta
        

        grad_counter = grad_counter.assign( grad_counter_new * tf.cast( (var-var_new) != 0, tf.float32) )
        var = var.assign( var_new )
        return var
  ...
  def _apply_centered_rmsprop(self,var,
        ...
        rms = rms.assign( rms + (tf.square(grad)-rms) * (1-decay_tensor) ) 
        mg = mg.assign( mg + (grad - mg) * (1-decay_tensor) )
        denom = tf.sqrt( rms - tf.square(mg) + epsilon_tensor)
        mom = mom.assign( ( (mom*momentum_tensor) + grad*learning_rate_tensor ) / denom )
        var = var.assign( var-mom )
        return var
