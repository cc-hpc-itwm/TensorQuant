...
from TensorQuant.Optimize import QSGD

import tensorflow as tf

class KfacOptimizer(QSGD.GradientDescentOptimizer):
  ...
  def __init__(self,
    ...
    Args:
      ...
      inv_devices: Iterable of device strings (e.g. '/gpu:0'). Inversion
          computations will be placed on these devices in a round-robin fashion.
          Can be None, which means that no devices are specified.
      quantizer: Intrinsic quantizer. Will be applied in superclass.
    ...
    super(KfacOptimizer, self).__init__(learning_rate, name=name, quantizer=quantizer)
...
def register_kfac(logits):
    """Automatically registers convolution and fully connected layers for kfac.
    Args:
        logits: logits of the classifier
    Return:
        A kfac tf.contrib.kfac.layer_collection.LayerCollection object.
    """
    ...
    def get_tensor(tensor_name):
        ...
        Returns a single tensor object, or None if there is no hit.
        """
        return tf.get_default_graph().get_tensor_by_name(tensor_name+':0')
        #for tensor in tf.get_default_graph().as_graph_def().node:
    ...
    kfac_valid_layers=["Conv2D", "MatMul"]

    # get all layers necessary for kfac
    node_list = [tensor for tensor in tf.get_default_graph().as_graph_def().node]
    for node in node_list:
        ...
        # Convolution layer
        if layer_type in kfac_valid_layers:
            tf.logging.info('K-FAC Item: %s, type: %s'%(node.name, node.op))
            reduced_node_name=reduce_hierarchy_list([node.name], -1)[0]
            # get weights
            for postfix in weights_postfixes:
                weights = get_variables_containing(reduced_node_name+'/'+postfix)
                if weights:
                    weights=weights[0]
                    break
            # get biases
            for postfix in biases_postfixes:
                biases = get_variables_containing(reduced_node_name+'/'+postfix)
                if biases:
                    biases=biases[0]
                    break
            if not biases:
                biases=tf.Variable(tf.zeros(weights.shape[3]))
                tf.identity(biases) # dummy operation to fool kfac
            
            # add layer to kfac collection, output logging info
            input = get_tensor(node.input[0])
            batch_size = input.shape[0]
            output = get_tensor(node.name)
            ...
            # register fully connected
                ...
                raise ValueError('kfac layer %s not recognized!'%node.name)

    # deal with BatchNorm parameters
    for var in tf.trainable_variables():
        if "BatchNorm" in var.name:
            if "BatchNorm/beta" in var.name:
                continue
            print("BatchNorm Variable: %s"%var.name)
            gamma=var
            beta=get_variables_containing(var.name.replace("BatchNorm/gamma","BatchNorm/beta"))[0]
            layer_collection.register_generic([gamma, beta],batch_size)
