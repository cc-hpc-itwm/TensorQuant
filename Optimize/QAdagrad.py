...
class QAdagradOptimizer(optimizer.Optimizer):
  ...
  def __init__(self, learning_rate, initial_accumulator_value=0.1,
               use_locking=False, name="QAdagrad", quantizer=None):
    ...
    Args:
      ...
      name: Optional name prefix for the operations created when applying
        gradients.  Defaults to "QAdagrad".
    ...
    super(QAdagradOptimizer, self).__init__(use_locking, name)
  ...
  def _prepare(self):
    self._learning_rate_tensor = ops.convert_to_tensor(self._learning_rate,
                                                ...
                                                name="learning_rate")
  def apply_qadagrad(self, var, acc, lr, grad):
    acc = acc.assign( acc+tf.square(grad) )
    delta =  grad * lr * tf.rsqrt(acc)
    var = var.assign( self.quantizer.quantize(var-delta) )
    return var
